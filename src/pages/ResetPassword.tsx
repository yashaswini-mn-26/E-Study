...
import { useParams, useNavigate } from 'react-router-dom'; // Need useParams for the token
import styles from '../styles/Login.module.css';

const ResetPassword: React.FC = () => {
  const { token } = useParams(); // Gets the token from /reset-password/:token
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    if (!passwordRegex.test(password)) {
      setError("Password must be at least 8 chars, 1 number, and 1 special character.");
      return;
    }
      ...
      <main className={styles.mainContent}>
        <div className={styles.formWrapper} style={{ justifyContent: 'center' }}>
          <div className={styles.formSection} style={{ maxWidth: '500px', width: '100%' }}>
            ...
            <p style={{ color: '#666', marginBottom: '1.5rem' }}>Please enter your new strong password below.</p>
            ...
            <form className={styles.loginForm} onSubmit={handleReset}>
              ...
              <button type="submit" className={styles.loginBtn}>Save New Password</button>
            </form>
          </div>
        </div>
      </main>
